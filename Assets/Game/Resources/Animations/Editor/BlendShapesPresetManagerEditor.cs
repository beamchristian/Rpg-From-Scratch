...
public class BlendShapesPresetManagerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (presetManager.showHelpBoxes)
        {
        ...
        presetManager.showSetup = EditorGUILayout.Foldout(presetManager.showSetup, "Setup & Options");
        if (presetManager.showSetup)
        {
            EditorGUI.indentLevel++;
            presetManager.showHelpBoxes = EditorGUILayout.Toggle("Show help boxes", presetManager.showHelpBoxes);
            presetManager.showFullInspector = EditorGUILayout.Toggle("Show full inspector", presetManager.showFullInspector);
        ...
        for (int i = 0; i < presetManager.presets.Count; i++)
        {
            ...
            if (preset.showValues)
            {
                ...
                for (int v = 0; v < preset.presetValues.Count; v++)
                {
                    ...
                    {
                        Undo.RecordObject(presetManager, "Undo Record");
        ...
        /* ------------------------------------------------------------------------------------------
         DEFAULT INSPECTOR
         ------------------------------------------------------------------------------------------*/
        if (presetManager.showFullInspector)
        {
            DrawLine();

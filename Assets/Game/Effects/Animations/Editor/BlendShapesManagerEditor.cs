...
public class BlendShapesManagerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (blendShapesManager.showHelpBoxes)
        {
            EditorGUILayout.HelpBox("BLEND SHAPES MANAGER\n" +
                                    "TO DO", MessageType.None);
        }
        
        /* ------------------------------------------------------------------------------------------
         SETUP AND OPTIONS
         ------------------------------------------------------------------------------------------*/
        blendShapesManager.showSetup = EditorGUILayout.Foldout(blendShapesManager.showSetup, "Setup & Options");
        if (blendShapesManager.showSetup)
        {
            // MAIN OPTIONS
            EditorGUI.indentLevel++;
            blendShapesManager.showHelpBoxes = EditorGUILayout.Toggle("Show Help Boxes", blendShapesManager.showHelpBoxes);
            blendShapesManager.showFullInspector = EditorGUILayout.Toggle("Show Full Inspector", blendShapesManager.showFullInspector);
        ...
        blendShapesManager.showRangeFiles = EditorGUILayout.Foldout(blendShapesManager.showRangeFiles, "Range Files (" + blendShapesManager.rangeFiles.Count + ")");
        if (blendShapesManager.showRangeFiles)
        {
            EditorGUI.indentLevel++;
            ...
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField("Export Path: " + blendShapesManager.exportPath);
            if (GUILayout.Button("Choose Path"))
            {
                blendShapesManager.exportPath = EditorUtility.SaveFolderPanel("Where should exports be saved?", "", "");
            }
            EditorGUILayout.EndHorizontal();
            EditorGUILayout.BeginHorizontal();
            if (blendShapesManager.exportPath != "")
        ...
        BLEND SHAPE OBJECTS
         ------------------------------------------------------------------------------------------*/
        DrawLine();
        blendShapesManager.showBlendShapeObjects = EditorGUILayout.Foldout(blendShapesManager.showBlendShapeObjects, "Blend Shape Objects (" + blendShapesManager.blendShapeGameObjects.Count + ")");
        if (blendShapesManager.showBlendShapeObjects)
        {
            EditorGUI.indentLevel++;
        ...
        /* ------------------------------------------------------------------------------------------
         DEFAULT INSPECTOR
         ------------------------------------------------------------------------------------------*/
        if (blendShapesManager.showFullInspector)
        {
            DrawLine(true);
    ...
    private void ExportRangeFile(BlendShapesManager blendShapesManager)
    {
        ...
        */
        if (!Directory.Exists(blendShapesManager.exportPath))
            System.IO.Directory.CreateDirectory(blendShapesManager.exportPath);
        
        System.IO.File.WriteAllText(blendShapesManager.exportPath + "/" + exportName + ".txt", exportString);
